...
メールを以下のカテゴリに分類:
- **要返信**: 自分からの返信が必要
- **要対応**: 返信以外のアクションが必要（書類作成、確認、承認等）
- **情報**: 読むだけでよい（CC、通知、お知らせ）
- **不要**: スパム、広告、不要な通知
- **至急**: 期限が近い、または緊急度が高い
- **保留**: 判断を後回しにする
      ...
      if (isLight) {
        ...
        if (mcpConfigPath) {
          args.push(
            ...
            '--allowedTools',
            'mcp__shirabe__get_unread_mails mcp__shirabe__get_recent_mails mcp__shirabe__get_mail_detail mcp__shirabe__get_mail_thread mcp__shirabe__get_accounts mcp__shirabe__analyze_thread mcp__shirabe__search_mails mcp__shirabe__tag_mail',
          );
        }
      }
