...
function searchForAccount(
  ...
  return withDbSync(acc.accountUid, acc.mailSubdir, 'mail_index.dat', (db) => {
    ...
    const rows = db
      ...
      .all(cutoffTicks, pattern, pattern, limit) as Array<{
      id: number;
      subject: string;
      date: number;
      preview: string;
...
export function searchMails(params: SearchMailsParams): { total: number; mails: MailSummary[] } {
  ...
  for (const acc of accounts) {
    try {
      const mails = searchForAccount(acc.email, params.keyword, cutoffTicks, params.limit);
      allMails = allMails.concat(mails);
    } catch (e) {
      console.error(`Error searching ${acc.email}:`, e);
    }
  }
